...
class ScreenRecorder:
    ...
    def start_recording(self):
        if not self.recording:
            ...
            output_file = os.path.join(self.output_dir, f"screen_recording_{timestamp}.mp4")
            self.video_writer = cv2.VideoWriter(output_file, self.fourcc, self.fps, (1920, 1080))
            self.recording = True
            self.frame_count = 0
